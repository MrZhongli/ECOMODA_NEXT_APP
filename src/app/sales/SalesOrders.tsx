...
export default function SalesData({ salesData }: SalesDataProps) {
    ...
    const getSaleTotal = (sale: Sale) => {
        return sale.saleDetail.reduce((total, item) => total + Number(item.unitPrice) * item.quantity, 0);
    };
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">
                                ${sales.reduce((sum, sale) => sum + getSaleTotal(sale), 0).toLocaleString()}
                            </div>
                        </CardContent>
                    </Card>
                </div>
                            ...
                            <TableBody>
                                {filteredSales.map((sale) => (
                                    <TableRow key={sale.id}>
                                        ...
                                        <TableCell>{sale.customer.name} {sale.customer.lastname}</TableCell>
                                        <TableCell>{new Date(sale.createdAt).toLocaleDateString()}</TableCell>
                                        <TableCell>${getSaleTotal(sale).toLocaleString()}</TableCell>
                                        <TableCell>
                                            <DropdownMenu>
